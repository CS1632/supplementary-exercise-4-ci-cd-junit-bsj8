...
public class RentACatImpl implements RentACat {
	...
	public boolean returnCat(int id) {
		// TODO
		Cat c=getCat(id);
		boolean available=catAvailable(id);
		if(c!=null && !available){		
			c.returnCat();
			return true;
	...
	public boolean rentCat(int id) {
		// TODO
		boolean exists=catExists(id);
		boolean available=catAvailable(id);
		if(exists && available){
			Cat c=getCat(id);
			c.rentCat();
			//System.out.println("rented is " + c.getRented());
			return true;
	...
	public String listCats() {
		...
		for(int i=0;i<cats.size();i++){
			...
			returnString+= "ID " +cats.get(i).getId()+ ". ";
			returnString+=cats.get(i).getName();
			returnString+="\n";
			}
		}
	...
	public boolean catExists(int id) {
		...
		if(cats.size()==0 || cats==null){
			...
			boolean result=cats.contains(getCat(id));
			return result;
		}
	}
